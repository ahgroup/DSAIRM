...
* For this app, the underlying function running the loop over different parameter values is called ``r simfunction``. This function in turn calls the bacteria model simulator function. You can call ``r simfunction`` directly, without going through the shiny UI. Use the `help()` command for more information on how to use the functions directly. If you go that route, you need to use the results returned from this function and produce useful output (such as a plot) yourself. 
* You could also modify this R function for your own purposes, e.g. such that it uses the virus model for model exploration.
