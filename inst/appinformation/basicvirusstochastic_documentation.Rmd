...
## Overview {#shinytab1}
This app allows exploration of a stochastic model that is almost identical to the deterministic model introduced in the __Basic Virus Model__ app. In fact, both models are being run so comparison is possible. Read about the model in the "Model" tab. Then do the tasks described in the "What to do" tab.
...
_Note that the `adaptivetau` package/function we use to run the stochastic model does not allow specification of starting time and time step. It always starts at *t=0* and each time step is chosen by the simulation routine based on the adaptivetau algorithm. Therefore to allow comparison with the ODE, the start time for the ODE is fixed to 0. Also the time step at which data is returned is fixed at tfinal/1000._
...
#########################
tid = tid + 1
tasktext = "With the settings as they just were, increase the number of simulations (for the stochastic model) to 2, re-run the simulation. Notice that you see two distinct outcomes for the two stochastic simulations. Also look at the message below the plot (what the app calls an outbreak is what I call an infection here, basically a within-host outbreak). Based on that, make sense of the fact that the average total number of infected cells for the stochastic simulation is much higher than for the deterministic (which contrasts to the finding from the previous task)."
nrec = 1 # number of items to record
out_records = c("Average total number of total infected cells for stochastic model")
out_types = rep("Integer",nrec)
out_notes = rep("Report the integer",nrec)
outcomes = data.frame( TaskID = rep(tid,nrec),
                       TaskText = rep(tasktext,nrec),
...
nrec = 1 # number of items to record
out_records = c("The average virus peak for the stochastic simulations that lead to an infection is above the ODE value (True/False)")
...
#########################
tid = tid + 1
tasktext = "Turn off the deterministic model, reset all parameter values to their starting values. Set number of simulations to 2, random number seed to 100. Run the simulation. You should see 2 stochastic model runs, one where you get an infection and one where you don't. Look at the min/max/final values reported. Then do 4 stochastic runs, look at min/max/final values again. From that, what do you conclude about how the mean is calculated? What other way of reporting it might be a reasonable alternative?"
nrec = 1 # number of items to record
...
#########################
tid = tid + 1
tasktext = "The starting number of virions has a strong impact on the chance an infection occurs. Some model parameters do as well. If the virus is fitter (e.g. higher virus production rate, faster rate of infection), the chances that an infection occurs also increases. Let's try that. Reset all inputs. Turn on stochastic simulations, set to 20. Run the model. You should now get 4 outbreaks/infections (the random number seed is different from above). Now set p=10. Re-run. The number of infections should have increased. Return to p=5 and set b=0.0002. You will again get more outbreaks/infections. Again, play with the random number seed and the number of simulations to convince yourself that this is a general finding."
...
#########################
tid = tid + 1
tasktext = "With the model settings we explored so far, stochasticity/randomness matters in deciding if an infection occurs or not. Once it has started, the resulting time-series look fairly smooth and not too noisy. The reason for that is that the numbers of virus growh quickly, and there are a lot of cells, thus in general the law of large numbers applies and the overall behavior of the system is fairly well approximated by a deterministic model. We can change this by reducing the number of cells. Reset all inputs. Then set U=1000. To make up for this change, increase b to b=0.002. Run 20 simulations of the stochastic model. You will see that the time-series looks much noisier, even at the peak. You should find that 4 simulations produce an infection. Record the average number of peak/maximum infected cells.
...
#########################
tid = tid + 1
tasktext = "Keep exploring. For instance investigate chronic/steady state conditions (with uninfected cell birth/death turned on) and see how they might or might not differ between the deterministic and (average) stochastic simulations."
nrec = 1 # number of items to record
out_records = c("Something")
out_types = rep("Rounded_Integer",nrec)
out_notes = rep("Report the rounded integer",nrec)
