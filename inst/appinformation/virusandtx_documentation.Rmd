...
#########################
tid = 1
tasktext = "For the first few tasks, we consider an acute viral infection and treatment (e.g. influenza and neuraminidase inhibitor drugs). The model is assumed to run in units of days.
...
#########################
tid = tid + 1
tasktext = "* Set 'start at steady state' back to 'no'. 
...
#########################
tid = tid + 1
tasktext = "* If you knew the drug would be administered at day 4 and it reduced infection of cells by 50%, what do you predict the maximum number of infected cells would be?
...
#########################
tid = tid + 1
tasktext = "For the next few tasks, we consider a chronic viral infection and treatment (e.g. HCV or HIV). The model is assumed to run in units of days.
...
#########################
tid = tid + 1
tasktext = "* Now set 'start at steady state' to 'yes'. Note that this will ignore the starting values for _U_, _I_ and _V_ and instead compute the steady state from the model parameters (using the equations you learned about in the 'Basic Virus' app).
...
ref figure {r hcvdata,  fig.cap='Source: Neumann et al 1998 Science.',  echo=FALSE}
...
#########################
tid = tid + 1
tasktext = "* Keep parameter settings as in the previous task. For a drug that reduced the rate of infection of cells by 80%, what virus load do you predict on day 30 (with treatment start on day 10)?  
...
#########################
tid = tid + 1
tasktext = "* Run the simulations from task 6 for a longer time and observe what the different variables do.
