...
The flow diagram and the equations are as follows:
...
Note that the drug is not explicitly modeled here (i.e. it is not given its own compartment). Instead, we will model the potential impact of the drug by its effects on some of the model parameters, and assume that this effect is either absent or present but does not otherwise change with time. The pharmacokinetics and pharacodynamics provides a model that includes explicit modeling of drug uptake and decay.
...
* If you could decide to design a drug to either reduce cell infection by 50% or virus production by 50% (and that drug was given on day 4), which drug would you prefer?
* If you could decide to either double drug efficacy for reducing virus production from 40% to 80%, or decide to give drug on day 2 instead of day 4, which alternative would you prefer?
* Can you find a scenario (i.e. values for the different model parameters) in which the 2 instead of 4 day approach beats the 40% vs 80% approach?
...
* Set all parameters as in task 1, but now allow for uninfected cell births (10000 per day) and cell death (rate of 0.1)
* Run the simulation for 100 days. You should reach a steady state with 35000 virions present.
