...
This model is of course very simple and abstract. However, as you will see, even with just 2 entities and their interactions, we can get interesting dynamics. This model could be extendend by introducing further compartments. For instance we could include separate compartments for the innate and adaptive immune response. You will encounter more detailed models in some of the other apps.
...
###Model Diagram
A very good way to describe compartmental models and move from a verbal description toward a mathematical/computational formulation is by using diagrams. Being able to go back and forth between verbal description, diagram and mathematicla/computatinal model is a crucial skill when building models. 
...
* 100 initial bacteria, an initial immune response strength of 1, a simulation time of 100 (which we assume to be days)
...
Run the simulation, see what you get. You should see some oscillations and then the system settles down, with about bacteria and immune response at the end of the simulation at around 19996 and 3003. 
...
* Set plotting now to 'both', which runs and shows both the continuous-time and discrete-time models.
...
You should get a plot where bacteria numbers drop to below 10^-4^. Contemplate what that means. This is one of the draw-backs of ODE based models, and we'll revisit this topic in the stochastic apps.
...
* You could also modify the simulator functions and adopt them for your own purposes. All simulator functions can be found in a subfolder of the package called `simulatorfunctions`. Of course to modify these functions, you'll need to do some coding.
* For examples on using the simulators directly and how to modify them, read the package vignette by typing `vignette('DSAIRM')` into the R console.
* The model we are exploring here belongs to a class of well-studied models in ecology known as predator-prey models. If you want to learn more about these kinds of models, see e.g. [@]. The models in those references are described in the context of ecology, but results transfer to within-host situation. For a similar model in the context of a within-host system, see e.g. [@handel09b].
