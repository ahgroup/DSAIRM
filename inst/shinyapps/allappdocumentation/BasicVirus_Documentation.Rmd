...
###Task 8: 
* Change around values of the parameters _b_, _p_, _d~V~_ and _d~I~_ and investigate how the steady state values for _U_, _I_ and _V_ depend on those parameters. You might need to increase the simulation time to ensure the system has settled down.
...
$0 = n - d_U U - bUV$, $0 = bUV - d_I I$, $0 = pI - d_V V - gb UV$. One can solve those equations for each of the compartments to get a mathematical expression of what _U_, _I_ and _V_ are at steady state. Try to do this. You should find that $U_s = d_I d_V/(b(p- d_I g))$ and similar equations for $I_s$ and $V_s$.
