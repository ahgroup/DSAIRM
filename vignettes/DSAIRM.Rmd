...
**NOT YET ON CRAN, USE BELOW METHOD* **
...
Package installation is a one-time process, unless R itself is being upgraded/reinstalled. Note that DSAIRM depends on other packages, which will also be installed as needed.



## Using the package
The following sections describe the main envisioned ways the content in this R package can be used and extended. The idea is that everyone starts at level 1, and then, based on needs and interests, students can decide to move on to the next level.
...
To facilitate direct interaction and modification of the underlying simulations, each app is structured in such a way that the underlying model/simulation is a stand-alone function. For some apps, there are multiple underlying functions involved, for instance the fitting function apps call one of the model simulation functions. You can call/use any of these functions directly, without going through the graphical interface. The 'Further Information' tab inside each app provides the name of the corresponding simulator function.
