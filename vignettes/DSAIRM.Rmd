...
```{r eval=FALSE, echo=TRUE}
help('simulate_Basic_Bacteria_model_ode')
```
...
result <- simulate_Basic_Bacteria_model_ode(vars = c(B = 500, I = 5), pars = c(g = 1, Bmax = 1e+06, dB = 0.1, k = 1e-07, r = 0.002, dI = 1) )
```
...
for (n in 1:length(rvec))
{
  ...
  result <- simulate_Basic_Bacteria_model_ode(  r = rvec[n] )
...
```{r eval=FALSE, echo=TRUE}
simulate_Basic_Bacteria_model_ode <- function(B = 10, I = 1, g =  1, Bmax = 1e+06, dB = 0.1, k = 1e-07, r = 0.001, dI = 1, tstart = 0, tfinal = 30, dt = 0.01)
```
...
We could rename the function that contains the equations, `Basic_Bacteria_model_ode_fct`, but that is not required.
