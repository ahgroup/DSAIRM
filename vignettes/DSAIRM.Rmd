...
result <- simulate_basicbacteria(B0 = 500, I0 = 5, tmax = 100, g = 1,  r =0.002)
```
...
```{r, eval=TRUE, echo=TRUE}
rvec = 10^seq(-4,2,length=10) #values of immune activation rate, r, for which to run the simulation 
...
for (n in 1:length(rvec))
{
  ...
  result <- simulate_basicbacteria(B0 = 10, I0 = 1, tmax = 100, r = rvec[n])
  peak[n] <- max(result[,"B"]) #record max number of infected for each value of r
}
#plot final result
plot(rvec,peak,type='p',xlab='Immune activation rate',ylab='Max number of bacteria')
```
...
The following individuals contributed to this package: 
