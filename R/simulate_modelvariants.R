...
virusandirode <- function(t, y, parms)
{
   with(
    as.list(c(y,parms)), #lets us access variables and parameters stored in y and parms by name
    {
      ...
      dUdt = - b*V*U - k3*F*U
      dIdt = b*V*U - dI*I - k2*F*I - k4*A*I - k6*A*I/(A+sA)
      dVdt = p*I/(1+k1*F) - dV*V - b*V*U - k5*A*V
      dFdt = pF - dF*F + f1 * g*V*(Fmax - F) + f2 * g*V / (V+sV)*F + f3 * g*V*I / (V*I+sV)*F
      dAdt = a1 * F * V / (F * V + hV) * gA * A + a2*F*A + a3*V/(V+hV)*F
